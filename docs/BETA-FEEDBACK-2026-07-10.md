...
### PH-2 instant feedback + PH-1 quality highscore (shipped client-side; vision redeploy pending)
...
- **⚠️ Vision redeploy pending (blocked):** the `analyze_photo` edge action was updated in source to classify + return `coverage` (schema + prompt + fallback), but the **Supabase MCP needs re-auth** this session so it is NOT deployed. Client handles `coverage` as optional — until the redeploy, reference promotion runs on `qualityScore` alone (unknown coverage = neutral rank), so nothing breaks; the skin-priority soft lock simply activates once coverage starts arriving. Deploy `supabase/functions/ai-service` (currently source is ahead of the live v15) when the MCP is authed.
- Green: typecheck / lint / i18n (6) / vitest (109) / web export. Native camera flows are not web-previewable and no device build was available, so this batch is built + gated but not device-verified.
