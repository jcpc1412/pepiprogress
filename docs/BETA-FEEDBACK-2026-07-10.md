...
### A-2 correlation chip + A-5 warmer copy + P-5 chat cleanup (shipped; edge deployed v17)
- **A-2 (`pepi-chat.tsx`):** a "what moved together?" chip now routes to the insights **correlation** path (Haiku `tier: 'quick'`, same `buildInsightHistory` facade the charts + Analysis use), pulling the correlation read out of the buried Analysis section and into the chat where it is asked. Falls back to a plain "not enough logged yet" line when the data is too sparse.
- **A-5 (copy + prompt-cache):** the flat neutral verdict states now carry character. `verdict.explanation.building` and `.on_track` each gained one interpretive sentence (6 locales, instrument voice, no em dashes). The insights system prompt gained a **stable TONE instruction** (one interpretive "what it means for your goal" sentence per trend, warmer but hedged, character on neutral reads) so both Analysis and the Pepi Q&A read less like a lab printout. Stable phrasing = prompt-cache friendly (edge v17).
- **P-5 (`pepi-chat.tsx`):** composer gained horizontal + vertical padding and a bottom safe-area inset (breathing room above the tab bar); each new message **animates in** (fade + rise, reduce-motion aware); the send button has a press state; the header is a proper **hero** (`display` type, uppercase), and the subtitle line is gone. The manual **Clear** button is replaced with **session-close auto-clear** (OQ-1 option 1): the thread resets when the user has been away for 15 min, via app background/foreground OR leaving/returning the Pepi tab. History survives an active session, clears on real disengagement.
- Green: typecheck / lint / i18n (6) / vitest (109) / web export. Edge v17 verified byte-for-byte against local source.


### PH-2 instant feedback + PH-1 quality highscore (shipped; edge deployed v16)
- **PH-2 (instant post-capture read):** both capture components (`photo-capture.tsx` + `vision-camera-capture.tsx`) now fire an `onSaved(photoId)` callback and persist a `qualityScore` at save. `progress-photos.tsx` runs an instant read the moment a shot lands: an **unconditional celebration** (reduce-motion-aware spring on the result card), a deterministic **comparability badge** (tilt delta vs the prior reference, offline/instant), a **cheap Haiku fit sentence** + retake nudge (`checkFit`, fails open), and the immutable-baseline case ("Baseline set…"). The full **Sonnet `analyze_photo` auto-runs only on a scheduled milestone day** (reuses `photo-cadence.ts` next-scientific date); otherwise it stays behind the deep-comparison button. Locked decision: Haiku always + Sonnet on milestones.
- **PH-1 (quality highscore + skin priority):** new pure `photo-reference.ts` (`pickReference` + `isNewHighscore`, + `photo-reference.test.ts`, 9 tests) selects a promotable **working reference** ranked coverage-first (skin priority soft lock: `minimal` > unknown > `clothed`; once a minimal reference exists a clothed shot never displaces it), then quality, then recency. The **ghost overlay now anchors to the working reference** (not just the latest shot), while `baseline` (oldest) stays the immutable day-one compare anchor. A **"new quality best"** pill fires on the instant-read card when a save becomes the new best. `PhotoEntry` gained `qualityScore` + `coverage`; the vision service's `coverage` is persisted on each deep analysis.
- **Vision service (deployed v16, verified live):** the `analyze_photo` edge action now classifies + returns `coverage` (schema field is required + prompt + fallback), confirmed byte-for-byte against the live function. The client persists it on each deep analysis, so the skin-priority soft lock is now fully active. Coverage stays optional on the client, so any photo analyzed before v16 (no coverage) just falls to neutral rank in `pickReference`.
- Green: typecheck / lint / i18n (6) / vitest (109) / web export. Native camera flows are not web-previewable and no device build was available, so the on-device capture UX (celebration, instant card) is built + gated but not device-verified.
...
5. ~~**A-5 copy pass** + **A-2 correlation surfacing** + **P-5 cleanup**~~ **Shipped** (edge v17). All 14 beta-feedback items are now implemented.
