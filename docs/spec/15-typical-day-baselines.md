...
> **Status: implemented (code complete, green gate).** Core `src/lib/typical-day.ts` (pure + `typical-day.test.ts`, 23 tests); store fields `typicalBaselines`/`typicalPromptState` + actions (`setTypicalBaseline`, `recordTypicalDeviation`, `silentFillTypical`, `clearTypicalHistory`, `setTypicalPromptState`) with manual/synced precedence; detailed-log usual/less/more chips + enter-exact escape hatch; Settings "Typical day" card (`typical-day-settings.tsx` + `/typical-day` route); Pepi conversational setup + opener chip + deterministic free-text deviation matching + silent fill; one-shot notification (`notifyTypicalPrompt`) with deep-link to Pepi. Because `caloric_balance`/`protein_adequacy` are insight-only, typical nutrition readings flow into the verdict as ordinary nutrition signals (no analysis-pipeline change needed); community exclusion is automatic since `metricReadings` never migrate to the normalized community tables. Deferred within this feature: ledger-event extraction for chip days (the signal already rides charts + verdict), and an edge-function `typical_deviation` parse (handled deterministically client-side instead).
